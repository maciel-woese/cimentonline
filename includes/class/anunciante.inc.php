...
class anunciante {
	...
	function setPostValues(){
		...
		if(isset($_POST['anun_codigo']))        $this->anun_codigo	   =	$_POST['anun_codigo'];
		...
		if(isset($_GET['anun_codigo']))       $this->anun_codigo	   =	$_GET['anun_codigo'];
	...
	function insert(){
		...
		if($this->nome == "")  {
			...
			if ($this->email != ""){
				...
				$emailVer = "SELECT COUNT(*) AS count FROM tb_anunciante WHERE anun_email= '".$this->email."'";
				...
				else
				{
					...
					$sql	= "INSERT INTO `tb_anunciante` 
								...
								VALUES
								(
									'".htmlentities($this->nome)."', 
									'".htmlentities($this->nome_empresa)."', 
									'".htmlentities($this->cargo)."', 
									'".$this->celular."', 
									'".$this->telefone."', 
							...
							if ( !$this->db->query($sql) )	{
								$erroTXT = "Erro ao cadastrar Anunciante, tente novamente.";
								$erro = 2;
							}
	...
	function listAll(){
		...
		$sql	= "SELECT a.anun_codigo, a.anun_dsc, a.anun_endereco, a.anun_email, a.anun_site, 
					a.anun_comentario, a.anun_tel, a.anun_cel 
					FROM tb_anunciante";
		...
		if (!empty($this->valorFor) and !empty($this->type)){
			...
			$count = "SELECT COUNT(*) AS count FROM tb_anunciante f
		             inner join estados e on e.sigl_estado = f.estado
	...
	function listFornecedor24($p1, $p2){
	   ...
	   if(isset($_POST['query']) and !empty($_POST['query'])){
        	$sql   .= " and f.for_dsc LIKE '%{$_POST['query']}%' ";
        	$count .= " and f.for_dsc LIKE '%{$_POST['query']}%' ";
        }
        if(isset($_POST['uf']) and !empty($_POST['uf'])){
        	$sql   .= " and f.est_codigo = '{$_POST['uf']}' ";
        	$count .= " and f.est_codigo = '{$_POST['uf']}' ";
        }
        if(isset($_POST['city']) and !empty($_POST['city'])){
        	$sql   .= " and f.cid_codigo = '{$_POST['city']}' ";
        	$count .= " and f.cid_codigo = '{$_POST['city']}' ";
        }
        if(isset($_GET['sigla']) and !empty($_GET['sigla'])){
        	$sqlB = "select est_codigo from tb_estado where est_sigla = '{$_GET['sigla']}'";
